...
export default function Home() {
  ...
  // Auth — mounted guard prevents hydration flash (skeleton until hydrate)
  ...
  const canAuth = email.trim() !== "" && password !== "" && !authLoading;

  const [stats, setStats] = useState<{ clicks: number; expired: boolean } | null>(null);

  const emitStorage = () => window.dispatchEvent(new Event("local-storage"));
  ...
  return (
    <div className="flex min-h-screen flex-col items-center p-8">
      ...
      {/* Auth bar — skeleton until mounted, then stable */}
      <div className="w-full max-w-lg mb-6 p-4 border rounded-lg bg-gray-50 min-h-[110px]">
        {!mounted ? (
          ...
          <div className="flex justify-between items-center">
            ...
            <button type="button" onClick={doLogout} className="text-sm text-red-500 underline">
              Logout
            </button>
          </div>
        ) : (
          <>
            ...
            <div className="flex gap-2 mt-2">
              <button
                ...
                onClick={doLogin}
                disabled={!canAuth}
                className="px-4 py-1.5 bg-blue-500 text-white rounded text-sm disabled:opacity-40 disabled:cursor-not-allowed"
              >
                {authLoading ? "..." : "Login"}
              </button>
              <button
                ...
                onClick={doRegister}
                disabled={!canAuth}
                className="px-4 py-1.5 bg-gray-200 rounded text-sm disabled:opacity-40 disabled:cursor-not-allowed"
              >
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-lg space-y-3">
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="custom ID (optional, 3-20 chars)"
      ...
      {shortUrl && (
        <div className="mt-4 p-4 bg-gray-100 rounded-lg w-full max-w-lg space-y-2">
          ...
          <a href={shortUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline font-mono text-sm break-all">
            {shortUrl}
          </a>
          {expiresAt && <p className="text-xs text-gray-500">Expires: {new Date(expiresAt).toLocaleString()}</p>}
          <div className="flex gap-2 pt-2">
            <button type="button" onClick={() => navigator.clipboard.writeText(shortUrl)} className="px-3 py-1 bg-white border rounded text-sm">
              Copy
            </button>
            <button type="button" onClick={() => fetchStats(shortId)} className="px-3 py-1 bg-white border rounded text-sm">
              Stats
            </button>
            {token && (
              <button
          ...
          {stats && (
            <p className="text-xs">
              Clicks: {stats.clicks} {stats.expired && <span className="text-red-500">(expired)</span>}
            </p>
          )}
        </div>
      )}
